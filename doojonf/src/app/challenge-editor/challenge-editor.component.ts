...
import { ChallengesService, CreatableChallenge } from '../challenge-services/challenges.service';
...
export class ChallengeEditorComponent implements AfterViewInit, OnInit {
  ...
  finish() {
    ...
    this._challenges.createChallenge(challenge).subscribe(res => {
      this._router.navigate(['/'])
    }, err => {
      this._snackbar.open('Sorry, something wrong happened', undefined, {duration: 3000})
    });
  }
