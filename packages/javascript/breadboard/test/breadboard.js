...
t.test("Circular dependency", t => {
  ...
  t.throws(
    ...
    new Error("circular dependency forbidden")
  );
  t.end();
});
...
t.test("Class as dependency path", t => {
  const container = new Container();
  ...
  container.addService("dependency", { class: Dependency });
  container.addService("dependent", { class: DependentClass });

  const instance = container.resolve(DependentClass);
  instance.callDependencyMethod();

  t.ok(isMethodCalled, "Dependency method called");

  t.end();
});

t.test("Class as dependency path when circular", t => {
  const container = new Container();

  class Dependency {
    static get deps() {
      return { dep: DependentClass };
    }
